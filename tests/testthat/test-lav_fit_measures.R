test_that("Returns scaled output when scaled test name provided", {
  ...
  fit.summary <- fitmeasures(fit_1, c(
...
test_that("Returns warning with invalid rmsea.ci.level.value", {
  ...
  model_1 <- "f =~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 "
  fit_1 <- cfa(model_1, data = data)
  expect_warning(fitmeasures(fit_1,
    fit.measures = "rmsea",
    fm.args = list(
      standard.test = "chisq",
      rmsea.ci.level = 100
...
test_that("Runs as expected if given scaled test", {
  ...
  model_1 <- "f =~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9"
  fit_1 <- cfa(model_1,
    data = data, test = c("mean.var.adjusted", "scaled.shifted"),
    ordered = c("x1")
  )
  fit.summary <- fitmeasures(fit_1,
    fit.measures = "all",
