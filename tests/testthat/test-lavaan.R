...
test_that("lavaan's model argument handles null formula", {
  expect_error(
    lavaan(model = NULL, data = HolzingerSwineford1939),
...
test_that("lavaan() handles invalid ordered argument", {
  expect_error(
    lavaan(model = "f =~ x1 + x2", data = data.frame(x1 = 1:10, x2 = 11:20), ordered = 123),
...
test_that("lavaan() handles invalid data types: function", {
  ...
  expect_error(
    lavaan(model, data = data_function),
...
test_that("lavaan() only takes character vector as ordered", {
  H.S.model <- "visual  =~ x1 + x2 + x3
            ...
            speed   =~ x7 + x8 + x9"
  expect_error(
    lavaan(H.S.model, data = HolzingerSwineford1939, ordered = 123),
...
test_that("lavaan's cluster argument only takes column name in the dataframe", {
  cluster_model <- "
    ...
    level: 2
        ...
        fb ~ w1 + w2"
  expect_error(
    lavaan(cluster_model, data = Demo.twolevel, cluster = "not_exist"),
...
test_that("lavaan's cluster argument is required when model contains level", {
  cluster_model <- "
    ...
    level: 2
        ...
        fb ~ w1 + w2"
  expect_error(
    lavaan(cluster_model, data = Demo.twolevel), # no cluster
...
test_that("lavaan() only takes logical as h1 in lavoption", {
  H.S.model <- "visual  =~ x1 + x2 + x3
            ...
            speed   =~ x7 + x8 + x9"
  expect_error(
    lavaan(H.S.model, data = HolzingerSwineford1939, h1 = 123),
