...
testthat::test_that("Model contains other kinds of operators", {
  ...
  res <- lav_standardize_all_nox(fit)
...
testthat::test_that("Returns correct class when only lavmodel and lavpartable provided", {
  ...
  expect_true(is.numeric(lav_standardize_all_nox(lavpartable = partable, lavmodel = FIT_CFA_HS@Model)))
...
testthat::test_that("Stops script if lavpartable$est is NULL", {
  ...
  expect_error(lav_standardize_all_nox(lavpartable = partable, lavmodel = FIT_CFA_HS@Model),
